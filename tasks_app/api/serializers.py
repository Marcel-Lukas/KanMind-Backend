...
class TaskSerializer(serializers.ModelSerializer):
    ...
    def validate(self, attrs):
        ...
        self._validate_user_membership(attrs.get('assignee'), board, 'Assignee')
        self._validate_user_membership(attrs.get('reviewer'), board, 'Reviewer')
    ...
    def _validate_user_membership(self, user, board, role):
        ...
        if not (is_owner or is_member):
            raise serializers.ValidationError(
                f'{role} must be a member or owner of the board.'
            )
    ...
    def _prevent_board_change(self, attrs):
        ...
        if self.instance and 'board' in attrs and attrs['board'] != self.instance.board:
            raise serializers.ValidationError(
                'Changing the board of a task is not allowed'
            )
